...
DWB รองรับ MCP session หลายตัวที่ bind workspace เดียวกัน พร้อม task board สำหรับแยกงานและจองขอบเขตไฟล์ แต่ละ agent ลงทะเบียนผ่าน `dwb_agent` แล้วใช้ `dwb_task` เพื่อสร้าง, claim, dispatch, complete, handoff, block, cancel หรือ release งาน เมื่อ complete agent สามารถส่ง summary, changed files, ผลทดสอบ และ result ให้ agent ถัดไปอ่านด้วย `dwb_task action=handoff` งานที่ระบุ role/capability จะถูก dispatch ให้ agent ที่ตรงเงื่อนไขโดย broker การ claim task ที่มีขอบเขตไฟล์ทับกับ task ที่กำลังทำอยู่จะถูกปฏิเสธ และ agent จะถูกบล็อกเมื่อพยายามเขียนไฟล์นอก scope ของ task ตัวเอง ระบบมี heartbeat/lease สำหรับคืนงานเมื่อ agent หลุด ดูตัวอย่างเต็มใน [คู่มือ Multi-Agent](docs/MULTI-AGENT-TH.md)
