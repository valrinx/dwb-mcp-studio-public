...
try{
  ...
  $expected='PASS: MCP Manager UI behavior: servers=3; enabled=2; running=1; issues=1; UTF-8 helper round-trip=verified; empty=visible; scroll=available; GitHub feedback=visible; progress=shown; window=fitted; layout=nonoverlapping'
  if($result.Trim() -ne $expected){throw "Expected '$expected'; got '$result'"}
  Write-Output 'MCP_MANAGER_UI_PASS: UTF-8 helper round-trip, summary, empty state, GitHub install feedback and progress, scrolling, display fitting, and section layout verified.'
}finally{
