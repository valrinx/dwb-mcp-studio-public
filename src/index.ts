...
async function createServer() {
  ...
  const server = new Server(
    { name: 'dwb-mcp-studio-core', version },
    {
      ...
      instructions:
        'DWB Core manages isolated Desktop Commander workers. When the user specifies a working directory or project root, immediately call workspace with action="bind" and path set to that absolute directory before any file or shell work. This one call registers or reuses the exact workspace and binds it to this chat; do not ask the user to separately create a workspace or repeat the directory. Verify the returned workingDirectory. If only a known project name is given, bind using its workspace name or alias. If no directory or project is given, do not infer it from other chats or from a file merely being read. Binding changes this session worker CWD; existing active processes must finish before switching. It does not expand the configured filesystem policy. Use absolute paths for filesystem tools. Re-read and reconcile files when stale-write protection reports a conflict. A DWB session ID is not a ChatGPT conversation ID. For complex user requests, register this session as the main agent with dwb_agent action=register, then split independent work into non-overlapping scopes and use dwb_task action=delegate to create and dispatch worker tasks in one call. When a task_assigned or task_handoff agent notification arrives, acknowledge it immediately, inspect the task/handoff, perform or continue the assigned work, and complete with summary, changed_files, test_result, and result so the next agent or main agent can continue. Use dwb_agent action="send" for durable progress/questions to another agent. After completing a task, call dwb_agent action="wait"; if it returns timedOut with no messages, immediately call wait again while this worker is available instead of ending the turn or polling inbox. The broker automatically wakes paused agent records whose MCP session is still connected, dispatches queued work, and sends notifications. MCP notifications are transport signals; the connected host must support server notifications and allow the agent to run the follow-up tool calls. If the host closes the MCP session after a response, DWB cannot start a new host conversation by itself; the host must reconnect or run a persistent worker loop.',
    },
  );
